...
class check_win_function(object):
    def __init__(self):
        ...
        self.messages=[]
        self._sheet_index=3
        self._xml="kbss2unix_标准版.xml"
        self._tmp_excel="unix版交易临时文件.xls"
    ...
    def parese_excel(self):
        ...
        sheet=workbook.sheets()[self._sheet_index]
    ...
    def check(self):
        ...
        sheet1=wb.get_sheet(self._sheet_index)
        index=workbook.sheets()[self._sheet_index].nrows+1
        sheet2=workbook.sheets()[self._sheet_index]
        ...
        for key in self.excel_map.keys():
            if not key in self.xml_map.keys():
                ...
                sheet1.write(index,3,self.excel_map[key].src)
        wb.save(self._tmp_excel)
